...
By default, CostHQ stores all session data in `~/.codesession/sessions.db`. Set the `CODESESSION_DB_PATH` environment variable to use a different location — useful for test isolation or keeping separate databases per project.
...
| --- | --- |
| macOS / Linux | `~/.codesession/sessions.db` |
