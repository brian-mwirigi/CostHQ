...
<Steps>
  ...
  <Step title="Install CostHQ globally">
    Run the global install command. This installs the `cs`, `codesession`, and `codesession-mcp` binaries on your PATH.
...
Installing `costhq` places three binaries on your system:

| Binary | Purpose |
| --- | --- |
| `cs` | Main CLI — all session and tracking commands |
| `codesession` | Alias for `cs` |
| `codesession-mcp` | MCP server for Claude Code integration |

Use `cs` for day-to-day work. Use `codesession-mcp` when [configuring Claude Code's MCP settings](/integrations/claude-code).

## Custom Database Path

By default, CostHQ stores all session data in `~/.CostHQ/sessions.db`. Set the `CODESESSION_DB_PATH` environment variable to use a different location — useful for test isolation or keeping separate databases per project.
...
# Windows PowerShell
$env:CODESESSION_DB_PATH = "C:\projects\client-a\.codesession.db"
```
