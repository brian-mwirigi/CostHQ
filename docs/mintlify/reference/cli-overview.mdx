...
The `cs` CLI is the main interface to CostHQ. Installing the `costhq` package globally gives you two aliases — `cs` and `codesession` — that point to the same binary. Every command in this reference works identically with either alias.
